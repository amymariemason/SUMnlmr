...
#' be used in the model. For piecewise_mr this can be a character string naming
...
#' localised average causal effect (LACE). The default is deciles
#' (i.e. 10 quantiles).
...
#' @param ci_quantiles the number of quantiles at which confidence intervals are
...
#' effect. The default is the mean of the x-values within each quantile,
#' otherwise specify a percentile (e.g. 0.5 corresponds to the median value).
#' @return List of non-linear MR results from the piecewise MR approach.
#' @return \item{n}{number of individuals.}
#' @return \item{model}{the model specifications. The first column is the number
#'  of quantiles (q); the second column is the number of bootstrap replications
#'  performed (nboot).}
#' @return \item{coefficients}{the regression estimates. The first column is the
...
#' @return \item{lace}{the localised average causal effect estimate in each
#' quantile. The first column is the regression coefficients (beta); the second
#'  column is the standard errors of the regression coefficients (se).}
#' @return \item{xcoef}{the association between the instrument and the exposure
#' in each quantile. The first column is the regression coefficients (beta); the
#'  second column is the standard errors of regression coefficients (se).}
#' @return \item{p_tests}{the p-value of the non-linearity tests. The first
#' column is the p-value of the test between the fractional polynomial degrees
#' (fp_d1_d2); the second column is the p-value from the fractional polynomial
#' non-linearity test (fp); the third column is the p-value from the quadratic
#' test (quad); the fourth column is the p-value from the Cochran Q test (Q).}
#' @return \item{p_heterogeneity}{the p-value of heterogeneity. The first column
#'  is the p-value of the Cochran Q heterogeneity test (Q); the second column is
#'   the p-value from the trend test (trend).}
#' @examples
...
piecewise_mr <- function(y, x, g, covar=NULL, family="gaussian", q=10, xpos="mean", nboot=100, fig=T, ref=mean(x), pref_x="x", pref_x_ref="x", pref_y="y", ci_quantiles=10, breaks=NULL){
  ...
  ##### Non-linearity tests #####
  p_quadratic <- metafor::rma(coef ~ xmean, (coef_se)^2, method="FE")$pval[2]
  p_Q <- 1 - pchisq(metafor::rma(coef, vi=(coef_se)^2)$QE, df=(q-1))
...
#' @author James R Staley <js16174@bristol.ac.uk>
#' @export
summary.piecewise_mr <- function(object, ...){
  ...
  if(is.null(object$figure)){summ <- list(model=model, n=n, coefficients=coefficients, p_tests=p_tests, p_heterogeneity=p_heterogeneity)}
  if(!is.null(object$figure)){summ <- list(model=model, n=n, coefficients=coefficients, p_tests=p_tests, p_heterogeneity=p_heterogeneity, figure=object$figure)}
