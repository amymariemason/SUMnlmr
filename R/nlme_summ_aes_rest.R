...
#' the function may error as several fractional polynomials provide the same fit.
#'
...
#' quantile of x.
#' @param ci_quantile the number of quantiles at which confidence intervals are
#'  to be displayed. The default is deciles (i.e. 10).
...
## - fix in line 204 to correct a typo; add $x to give log(plot.data$x) in the
## second from last term

frac_poly_summ_mr <- function(by, bx, byse, bxse, xmean, method="FE", d=1,
                              powers=c(0, -3, -2, -1.5, -1, -0.5, 1, 2),
                              pd=0.05, ci="model_se", nboot=100, fig=FALSE,
                              family="binomial", offset=0, pref_x="x",
                              pref_y="y", ref=NA, ci_type="overall",
                              breaks=NULL, ylim_lower = NA, ylim_upper = NA,
                              xlim_lower = NA, xlim_upper = NA) {
  ##### Error messages #####
  if (!(d==1 | d==2 | d=="both")) stop('the degree has to equal 1, 2 or "both"')
  ...
  p_het <- 1- stats::pchisq(metafor::rma(xcoef_sub,
                                         vi = (xcoef_sub_se)^2)$QE, df = (q-1))
  p_het_trend <- metafor::rma.uni(xcoef_sub ~ xmean,
                                  vi = xcoef_sub_se^2,
  ...
  for (p1 in powers) {
    ...
    p[j]<-p1
    ML[j] <- summary(metafor::rma(frac_coef/xcoef ~ -1 + x1, vi = (frac_se/xcoef)^2,
  ...
  if (d == 1 | d == 2 | d == "both") {
    ...
    for (p11 in powers1) {
      ...
      for (p21 in powers2) {
        ...
        p1[j] <- p11
        p2[j] <- p21
        cc <- try(metafor::rma(frac_coef/xcoef ~ -1 + x1 + x2,
                      vi = (frac_se/xcoef)^2,
                      method = method),
                      silent = TRUE)
        ...
        if (methods::is(cc, "try-error") == F) {
          ML[j] <- summary(metafor::rma(frac_coef/xcoef ~ -1 + x1 + x2,
                               vi = (frac_se/xcoef)^2,
                               method = method))$fit.stats[1,1]
          }
        j <- j+1
  ...
  if (d == 1 | d == 2 | d == "both") {
    ...
    best_fracp_d1 <- metafor::rma(frac_coef/xcoef ~ -1 + x1,
                         vi = (frac_se/xcoef)^2,
                         method = method)
    ...
    best_fracp_d2 <- metafor::rma(frac_coef/xcoef ~ -1 + x1 + x2, vi=(frac_se/xcoef)^2,
                         method = method)
    dev_best_fracp_d2 <- best_fracp_d2$fit.stats[2,1]
    p_d1_d2 <- 1 - stats::pchisq((dev_best_fracp_d1 - dev_best_fracp_d2), df=2)
    if (p_d1_d2>=pd) {
      d1 <- 1
    }else{
      d1 <- 2}
  ...
  if (d == 1) {
    ...
    model <- metafor::rma(frac_coef/xcoef ~ -1 + x1,
                 vi = (frac_se/xcoef)^2,
                 method = method)
  }
  if (d == 2) {
    ...
    model <- metafor::rma(frac_coef/xcoef ~ -1 + x1 + x2,
                 vi = (frac_se/xcoef)^2,
                 method = method)
  }
  ...
  if (ci == "bootstrap_per" | ci=="bootstrap_se") {
    if (d == 1) {
      ...
      for (i in 1:nboot) {
        ...
        mod <- metafor::rma.uni(frac_coef1/xcoef ~ -1 + x111,
                       vi = (frac_se1/xcoef)^2,
                       method = method)
      ...
      for (i in 1:nboot) {
        ...
        if (p1_ML == p2_ML) {
          if (p2_ML == -1) {
            ...
            x211 <- ((p2_ML + 1)*(xmean1^p2_ML)*log(xmean1) + xmean1^p2_ML)
          }
        }else{
          if (p2_ML == -1) {
            x211 <- xmean1^p2_ML}else{x211 <- (p2_ML + 1)*xmean1^p2_ML
          }
        }
        mod <- metafor::rma.uni(frac_coef1/xcoef ~ -1 + x111 + x211,
                       vi = (frac_se1/xcoef)^2,
                       method = method)
  ...
  linear <- metafor::rma(frac_coef/xcoef ~ 1,
                vi = (frac_se/xcoef)^2,
                method = method)
  ...
  ##### Other tests #####
  p_quadratic <- metafor::rma(frac_coef/xcoef ~ xmean,
                     vi = (frac_se/xcoef)^2,
                     method = method)$pval[2]
  p_Q <- 1 - stats::pchisq(metafor::rma(frac_coef/xcoef,
                        vi = (frac_se/xcoef)^2)$QE,
                        df = (q - 1))
  ...
  if (fig == TRUE) {
    if (ci_type == "overall"){
      ...
      plot.data <- data.frame(x = stats::runif(10000, min(xmean), max(xmean)))
      ...
      if (d == 2) {
        ...
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((log(plot.data$x)-log(ref))^2*cov[1,1] +
            2*(log(plot.data$x)-log(ref))*(log(plot.data$x)*log(plot.data$x)-
            log(ref)*log(ref))*cov[1,2] +
            (log(plot.data$x)*log(plot.data$x)-
          ...
          boot <- log(plot.data$x)%*%t(frac_coef_boot[,1]) +
            ...
            reprow(log(ref)%*%t(frac_coef_boot[,1]) +
                  log(ref)*log(ref)%*%t(frac_coef_boot[,2]),n = nrow(plot.data))
          plot.data$lci <- rowQuantiles(boot, probs = 0.025)
          plot.data$uci <- rowQuantiles(boot, probs = 0.975)
        }
      }
      if (p1_ML == -1 & p2_ML != -1 & p1_ML != p2_ML) {
        ...
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((log(plot.data$x) - log(ref))^2*cov[1,1] +
                  2*(log(plot.data$x) - log(ref))*(plot.data$x^(p2_ML+1) -
                  ref^(p2_ML + 1))*cov[1,2] +
                  (plot.data$x^(p2_ML+1) -
          ...
          boot <- log(plot.data$x)%*%t(frac_coef_boot[,1]) +
            ...
            reprow(log(ref)%*%t(frac_coef_boot[,1]) +
                     ref^(p2_ML+1)%*%t(frac_coef_boot[,2]), n = nrow(plot.data))
        plot.data$lci <- rowQuantiles(boot, probs = 0.025)
        plot.data$uci <- rowQuantiles(boot, probs = 0.975)
        }
      }
      if(p1_ML != -1 & p2_ML == -1 & p1_ML != p2_ML) {
        plot.data$yest <- beta[1]*plot.data$x^(p1_ML+1) +
          beta[2]*log(plot.data$x) - (beta[1]*ref^(p1_ML+1) + beta[2]*log(ref))
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((plot.data$x^(p1_ML + 1) -
                ref^(p1_ML+1))^2*cov[1,1] +
                2*(plot.data$x^(p1_ML+1)-ref^(p1_ML+1))*(log(plot.data$x)-
                  log(ref))*cov[1,2] + (log(plot.data$x) - log(ref))^2*cov[2,2])
          plot.data$lci <- plot.data$yest - 1.96*plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96*plot.data$yse
        }else{
          boot <- plot.data$x^(p1_ML + 1)%*%t(frac_coef_boot[,1]) +
            ...
            reprow(ref^(p1_ML+1)%*%t(frac_coef_boot[,1]) +
                     log(ref)%*%t(frac_coef_boot[,2]), n=nrow(plot.data))
          plot.data$lci <- rowQuantiles(boot, probs = 0.025)
          plot.data$uci <- rowQuantiles(boot, probs = 0.975)
        }
      }
      if (p1_ML != -1 & p2_ML != -1 & p1_ML == p2_ML) {
        ...
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((plot.data$x^(p1_ML + 1) -
              ref^(p1_ML + 1))^2*cov[1,1] + 2*(plot.data$x^(p1_ML+1)-
              ref^(p1_ML + 1))*(plot.data$x^(p2_ML+1)*log(plot.data$x)-
              ref^(p2_ML + 1)*log(ref))*cov[1,2] +
              (plot.data$x^(p2_ML+1)*log(plot.data$x)-
      ...
      if(p1_ML != -1 & p2_ML != -1 & p1_ML != p2_ML) {
        ...
        if (ci != "bootstrap_per"){
          plot.data$yse <- sqrt((plot.data$x^(p1_ML+1)-
              ref^(p1_ML + 1))^2*cov[1,1] + 2*(plot.data$x^(p1_ML+1)-
              ref^(p1_ML + 1))*(plot.data$x^(p2_ML + 1) -
              ref^(p2_ML + 1))*cov[1,2] + (plot.data$x^(p2_ML + 1) -
              ref^(p2_ML + 1))^2*cov[2,2])
          plot.data$lci <- plot.data$yest - 1.96*plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96*plot.data$yse
        }else{boot <- plot.data$x^(p1_ML + 1)%*%t(frac_coef_boot[,1]) +
          plot.data$x^(p2_ML + 1)%*%t(frac_coef_boot[,2]) -
          reprow(ref^(p1_ML + 1)%*%t(frac_coef_boot[,1]) +
                   ref^(p2_ML + 1)%*%t(frac_coef_boot[,2]), n=nrow(plot.data))
          plot.data$lci <- rowQuantiles(boot, probs = 0.025)
    ...
    ref = ref + offset
    if (family != "binomial") {
      figure <- ggplot2::ggplot(plot.data, aes(x = x))
      figure <- figure + geom_hline(aes(yintercept=0), colour="grey") +
    ...
    if(family == "binomial") {
      ...
      plot.data.1$y <- exp(0)
      figure <- ggplot2::ggplot(plot.data, aes(x = x))
      figure <- figure + geom_hline(aes(yintercept = 1), colour="grey") +
                  geom_line(aes(y=yest), color="black") +
                  geom_line(aes(y=lci), color="grey") +
                  geom_line(aes(y=uci), color="grey") +
                  theme_bw() +
  ...
  if (ci_type == "quantile"){
      ...
      plot.data.1 <- data.frame(x = stats::runif(10000, min(xmean), max(xmean)))
      if (d == 1){
        if (p_ML == -1) {
          plot.data$yest <- beta*log(plot.data$x) - (beta*log(ref))
          plot.data.1$yest <- beta*log(plot.data.1$x) - (beta*log(ref))
          if (ci != "bootstrap_per") {
            plot.data$yse <- sqrt((log(plot.data$x)-log(ref))^2*cov)
            plot.data$lci <- plot.data$yest - 1.96*plot.data$yse;
            plot.data$uci <- plot.data$yest + 1.96*plot.data$yse
      ...
      if (d == 2){
        if (p1_ML == -1 & p2_ML == -1) {
          plot.data$yest <- beta[1]*log(plot.data$x) +
            beta[2]*log(plot.data$x)*log(plot.data$x) -
            (beta[1]*log(ref) + beta[2]*log(ref)*log(ref))
          plot.data.1$yest <- beta[1]*log(plot.data.1$x) +
            beta[2]*log(plot.data.1$x)*log(plot.data.1$x) -
            (beta[1]*log(ref) + beta[2]*log(ref)*log(ref))
          if (ci != "bootstrap_per") {
            plot.data$yse <- sqrt((log(plot.data$x)-log(ref))^2*cov[1,1] +
              2*(log(plot.data$x)-log(ref))*(log(plot.data$x)*log(plot.data$x) -
              log(ref)*log(ref))*cov[1,2] + (log(plot.data$x)*log(plot.data$x) -
              log(ref)*log(ref))^2*cov[2,2]); plot.data$lci <- plot.data$yest -
              1.96*plot.data$yse
            plot.data$uci <- plot.data$yest + 1.96*plot.data$yse
          }else{
            boot <- log(plot.data$x)%*%t(frac_coef_boot[,1]) +
      ...
      plot.data$x = plot.data$x + offset
      plot.data.1$x = plot.data.1$x + offset
      if(family!="binomial"){figure <- ggplot2::ggplot(plot.data, aes(x=x)); figure <- figure + geom_hline(aes(yintercept=0), colour="grey") + geom_line(aes(x=x, y=yest), color="black", data=plot.data.1) + geom_errorbar(mapping=aes(x=x, ymin=lci, ymax=uci), color="grey", width=0.025) + geom_point(aes(y=yest), color=highlight, size=4) + theme_bw() + labs(x=pref_x,y=pref_y) + theme(axis.title.x = element_text(vjust=0.5, size=20), axis.title.y = element_text(vjust=0.5, size=20), axis.text.x=element_text(size=18), axis.text.y=element_text(size=18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()); if(!is.null(breaks)){figure <- figure + scale_y_continuous(breaks=breaks)}}
      if(family=="binomial"){plot.data$yest <- exp(plot.data$yest); plot.data$uci <- exp(plot.data$uci); plot.data$lci <- exp(plot.data$lci); figure <- ggplot2::ggplot(plot.data, aes(x=x)); plot.data.1$yest <- exp(plot.data.1$yest); figure <- figure + geom_hline(aes(yintercept=1), colour="grey") + geom_line(aes(x=x, y=yest), color="black", data=plot.data.1) + geom_errorbar(mapping=aes(x=x, ymin=lci, ymax=uci), color="grey", width=0.025) + geom_point(aes(y=yest), color=highlight, size=4) + theme_bw() + labs(x=pref_x,y=pref_y) + theme(axis.title.x = element_text(vjust=0.5, size=20), axis.title.y = element_text(vjust=0.5, size=20), axis.text.x=element_text(size=18), axis.text.y=element_text(size=18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()); if(!is.null(breaks)){figure <- figure + scale_y_continuous(breaks=breaks)}; figure <- figure + coord_trans(y="log")}
    }
